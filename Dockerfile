...
RUN yum -y install mysql-devel openssl-devel tre-devel tcp_wrappers-devel memcached-devel tre tcp_wrappers memcached catdoc libpst libzip sysstat poppler-utils unrtf tnef gcc make which cronie && \
  ...
  curl -L https://bitbucket.org/jsuto/piler/downloads/piler-1.1.1.tar.gz  | tar --strip-components=1 -xzC /usr/src/piler && \
  ...
  ln -sf /usr/local/etc/sphinx.conf /etc/sphinx/sphinx.conf && \
...
#VOLUMES ["/usr/local/etc/", "/var/piler/"]
